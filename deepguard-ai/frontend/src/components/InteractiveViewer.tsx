...
import { Eye, ShieldAlert, Zap, Layers, RefreshCw, Sparkles, Activity, Waves, Triangle, Grid3x3 } from 'lucide-react';
import { SampleMedia, AnalysisOverlay, DiagnosticImages } from '../types';
...
type DiagnosticMode = 'none' | 'ela' | 'fft' | 'edges' | 'wavelet' | 'dct';

const DIAGNOSTIC_LABELS: Record<DiagnosticMode, string> = {
  none: 'Original',
  ela: 'Error Level Analysis',
  fft: 'FFT Spectrum',
  edges: 'Edge Detection',
  wavelet: 'Wavelet HH Band',
  dct: 'DCT Coefficients',
};

const DIAGNOSTIC_KEYS: Record<DiagnosticMode, keyof DiagnosticImages | null> = {
  none: null,
  ela: 'ela',
  fft: 'fft',
  edges: 'edges_canny',
...
export default function InteractiveViewer({ media, isScanning, scanProgress, onReScan }: InteractiveViewerProps) {
  ...
  const [diagnosticMode, setDiagnosticMode] = useState<DiagnosticMode>('none');
  const [hoveredOverlay, setHoveredOverlay] = useState<AnalysisOverlay | null>(null);

  // Reset local state when media changes (new scan)
  useEffect(() => {
  ...
  const diagActive = diagnosticMode !== 'none';
  ...
  const renderFaceMesh = () => {
    if (!showLandmarks || diagActive || isScanning) return null;
  ...
  const diagImageKey = DIAGNOSTIC_KEYS[diagnosticMode];
  const diagBase64 = diagImageKey ? media.diagnosticImages?.[diagImageKey] : null;
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4 shadow-sm flex flex-col transition-colors duration-300">
      ...
      <div className="relative overflow-hidden rounded-xl bg-black aspect-video flex items-center justify-center border border-gray-150 dark:border-gray-800">
        ...
        {/* Diagnostic overlay image (actual base64 from backend) */}
        {diagActive && diagBase64 && !isScanning && (
          <img
            src={`data:image/jpeg;base64,${diagBase64}`}
            alt={DIAGNOSTIC_LABELS[diagnosticMode]}
                ...
                >
                  {isAnomaly ? (
                    <div 
                      ...
                      style={{
                        width: `${Math.max((overlay.w || 10) * 3, 20)}px`,
                        height: `${Math.max((overlay.h || 8) * 3, 16)}px`,
        ...
        {hoveredOverlay && !isScanning && !diagActive && (
      ...
      <div className="mt-4 border-t border-gray-100 dark:border-gray-800 pt-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          ...
          {/* Anomaly Areas */}
          <button
            onClick={() => { setShowAnomalies(!showAnomalies); setDiagnosticMode('none'); }}
            disabled={isScanning}
            className={`flex items-center space-x-2 px-3 py-2 rounded-xl text-left border text-xs font-medium transition ${
              showAnomalies && !diagActive
          ...
          {/* Lighting Angles */}
          <button
            onClick={() => { setShowLighting(!showLighting); setDiagnosticMode('none'); }}
            disabled={isScanning}
            className={`flex items-center space-x-2 px-3 py-2 rounded-xl text-left border text-xs font-medium transition ${
              showLighting && !diagActive
          ...
          {/* ELA */}
          <button
            onClick={() => setDiagnosticMode(diagnosticMode === 'ela' ? 'none' : 'ela')}
            disabled={isScanning}
            className={`flex items-center space-x-2 px-3 py-2 rounded-xl text-left border text-xs font-medium transition ${
              diagnosticMode === 'ela'
          ...
          {/* FFT */}
          <button
            onClick={() => setDiagnosticMode(diagnosticMode === 'fft' ? 'none' : 'fft')}
            disabled={isScanning}
            className={`flex items-center space-x-2 px-3 py-2 rounded-xl text-left border text-xs font-medium transition ${
              diagnosticMode === 'fft'
                ? 'bg-indigo-50 dark:bg-indigo-950/20 border-indigo-200 dark:border-indigo-950/30 text-indigo-700 dark:text-indigo-400'
                : 'bg-gray-50 dark:bg-gray-950 border-gray-200 dark:border-gray-850 hover:bg-gray-100 dark:hover:bg-gray-900 text-gray-600 dark:text-gray-400'
            }`}
            id="toggle-fft-btn"
          >
          ...
          {/* Edge Detection */}
          <button
            onClick={() => setDiagnosticMode(diagnosticMode === 'edges' ? 'none' : 'edges')}
            disabled={isScanning}
            className={`flex items-center space-x-2 px-3 py-2 rounded-xl text-left border text-xs font-medium transition ${
              diagnosticMode === 'edges'
                ? 'bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-950/30 text-amber-700 dark:text-amber-400'
                : 'bg-gray-50 dark:bg-gray-950 border-gray-200 dark:border-gray-850 hover:bg-gray-100 dark:hover:bg-gray-900 text-gray-600 dark:text-gray-400'
            }`}
            id="toggle-edges-btn"
          >
          ...
          {/* Wavelet HH */}
          <button
            onClick={() => setDiagnosticMode(diagnosticMode === 'wavelet' ? 'none' : 'wavelet')}
            disabled={isScanning}
            className={`flex items-center space-x-2 px-3 py-2 rounded-xl text-left border text-xs font-medium transition ${
              diagnosticMode === 'wavelet'
                ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-950/30 text-emerald-700 dark:text-emerald-400'
                : 'bg-gray-50 dark:bg-gray-950 border-gray-200 dark:border-gray-850 hover:bg-gray-100 dark:hover:bg-gray-900 text-gray-600 dark:text-gray-400'
            }`}
            id="toggle-wavelet-btn"
          >
            <Triangle className="w-3.5 h-3.5" />
            <span>Wavelet HH</span>
          </button>

          {/* DCT */}
          <button
            onClick={() => setDiagnosticMode(diagnosticMode === 'dct' ? 'none' : 'dct')}
            disabled={isScanning}
            className={`flex items-center space-x-2 px-3 py-2 rounded-xl text-left border text-xs font-medium transition ${
              diagnosticMode === 'dct'
                ? 'bg-cyan-50 dark:bg-cyan-950/20 border-cyan-200 dark:border-cyan-950/30 text-cyan-700 dark:text-cyan-400'
                : 'bg-gray-50 dark:bg-gray-950 border-gray-200 dark:border-gray-850 hover:bg-gray-100 dark:hover:bg-gray-900 text-gray-600 dark:text-gray-400'
            }`}
            id="toggle-dct-btn"
          >
        ...
        {/* Active diagnostic label */}
        {diagActive && (
          <div className="mt-2 px-3 py-1.5 bg-gray-100 dark:bg-gray-950 rounded-lg">
            <p className="text-[11px] font-mono text-gray-500 dark:text-gray-400">
              Viewing: <span className="font-semibold text-gray-700 dark:text-gray-300">{DIAGNOSTIC_LABELS[diagnosticMode]}</span>
              {diagBase64 ? ' — actual computed overlay from preprocessing pipeline' : ' — not available for this image'}
            </p>
          </div>
        )}
