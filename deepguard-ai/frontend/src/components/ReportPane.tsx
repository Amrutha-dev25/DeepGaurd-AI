...
import { ShieldCheck, ShieldAlert, ShieldQuestion, AlertTriangle, CheckSquare, FileText, Download, Share2, Clipboard, Globe } from 'lucide-react';
...
export default function ReportPane({ media, isScanning, scanProgress }: ReportPaneProps) {
  ...
  const isInconclusive = media.class === 'Inconclusive';
  const riskColor = 
    media.risk === 'High' 
  ...
  const handleCopyReport = () => {
    const resultLabel = isInconclusive ? 'INCONCLUSIVE' : (isFake ? 'FAKE' : 'REAL');
    const textToCopy = `DEEPFAKE FORENSICS REPORT
---------------------------
Exhibits: ${media.title}
Result: ${resultLabel}
Confidence Score: ${Math.round(media.confidence * 100)}%
  ...
  const handleDownload = () => {
    ...
    setTimeout(() => {
      try {
        ...
        doc.text(`REPORT_ID: ${media.id.slice(0, 16)}`, 16, 30);

        // 3. Verdict Highlight Box
          const isFakeMedia = media.class === 'Fake';
          const isInconcMedia = media.class === 'Inconclusive';
          const accent = isFakeMedia 
            ? { rgb: [244, 63, 94], label: 'DEEPFAKE ANOMALIES DETECTED' } 
            : isInconcMedia 
            ? { rgb: [245, 158, 11], label: 'INCONCLUSIVE / MANUAL REVIEW RECOMMENDED' }
        ...
        doc.text(`${Math.round(media.confidence * 100)}% Confidence Score`, 156, 64);
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border-2 border-gray-150 dark:border-gray-800 p-6 shadow-md transition-colors duration-300">
      ...
      <div className="space-y-6">
          ...
          <div className="md:col-span-2">
            ...
            <div className={`p-4 rounded-xl border flex items-center space-x-3.5 ${
              isFake
                ...
                : isInconclusive
                ? 'bg-amber-50/70 border-amber-150 text-amber-800 dark:bg-amber-950/20 dark:border-amber-950/30 dark:text-amber-450'
              ...
              <div className={`p-2.5 rounded-xl ${isFake ? 'bg-rose-100 dark:bg-rose-900' : isInconclusive ? 'bg-amber-100 dark:bg-amber-900' : 'bg-emerald-100 dark:bg-emerald-900'}`}>
                {isFake ? <ShieldAlert className="w-6 h-6" /> : isInconclusive ? <ShieldQuestion className="w-6 h-6" /> : <ShieldCheck className="w-6 h-6" />}
              </div>
              <div>
                ...
                <div className="text-2xl font-black tracking-tight mt-0.5">
                  {isInconclusive ? 'INCONCLUSIVE' : `${media.class.toUpperCase()} MEDIUM`}
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="relative w-12 h-12 flex-shrink-0">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle
                    ...
                    cy="24"
                    r="20"
                    className={`stroke-[4] transition-all duration-1000 ${isFake ? 'stroke-rose-500' : isInconclusive ? 'stroke-amber-500' : 'stroke-emerald-500'}`}
                    fill="none"
                    strokeDasharray={`${2 * Math.PI * 20}`}
                    strokeDashoffset={`${2 * Math.PI * 20 * (1 - media.confidence)}`}
                  />
                </svg>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {media.findings.map((finding, idx) => (
              ...
              >
                <span className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${isFake ? 'bg-rose-500' : isInconclusive ? 'bg-amber-500' : 'bg-emerald-500'}`} />
                <span>{finding}</span>
              </li>
            ))}
          </ul>
        ...
        {media.findings.some(f => f.startsWith('[Conflict]')) && (
          <div className="bg-amber-50 dark:bg-amber-950/20 p-4 rounded-xl border border-amber-200 dark:border-amber-900/30">
            ...
            <ul className="space-y-1.5">
              {media.findings.filter(f => f.startsWith('[Conflict]')).map((item, idx) => (
                <li key={idx} className="text-xs text-amber-600 dark:text-amber-400 leading-relaxed">
                  {item.replace('[Conflict] ', '')}
                </li>
              ))}
            </ul>
          </div>
